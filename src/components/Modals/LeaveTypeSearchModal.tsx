...
export function LeaveTypeSearchModal({
    ...
    const [leaveTypes,  setLeaveTypes]  = useState<{ code: string; description: string }[]>([]);
    ...
    useEffect(() => {
        ...
        apiClient.get('/Fs/Process/LeaveTypeSetUp')
            .then(res => {
                const mapped = (res.data as any[]).map(lt => ({
                    code:        lt.leaveCode ?? '',
                    description: lt.leaveDesc ?? '',
                }));
                    ...
                    <div className="p-3 overflow-y-auto flex-1">
                        ...
                        <div className="border border-gray-200 rounded">
                            <table className="w-full border-collapse text-sm">
                                ...
                                <tbody>
                                    {loading ? (
                                        <tr><td colSpan={2} className="px-3 py-4 text-center text-gray-500 text-sm">Loading...</td></tr>
                                    ) : error ? (
                                        <tr><td colSpan={2} className="px-3 py-4 text-center text-red-500 text-sm">{error}</td></tr>
                                    ) : paged.length === 0 ? (
                                        <tr><td colSpan={2} className="px-3 py-4 text-center text-gray-500 text-sm">No data available</td></tr>
                                    ) : (
                                        paged.map(lt => (
                                            <tr key={lt.code} className="border-b border-gray-200 hover:bg-blue-50 cursor-pointer"
                                                onClick={() => { onSelect(lt.code, lt.description); onClose(); }}>
