...
interface AllowanceCodeSearchModalProps {
  show: boolean;
  onClose: () => void;
...
export function AllowanceCodeSearchModal({
  show,
  onClose,
  onSelect,
  searchTerm,
  setSearchTerm,
}: AllowanceCodeSearchModalProps) {
  const [allowances,  setAllowances]  = useState<{ code: string; description: string }[]>([]);
  const [loading,     setLoading]     = useState(false);
  const [error,       setError]       = useState('');
  const [currentPage, setCurrentPage] = useState(1);

  // ── Fetch allowance codes from API ───────────────────────────────────────
  useEffect(() => {
    if (!show) return;
    fetchAllowances();
  }, [show]);
  ...
  const fetchAllowances = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        console.log('Allowance raw response:', response.data[0]); // 👈 remove once field names confirmed
        const mapped = (response.data as any[]).map(a => ({
          code:        a.code        ?? a.allowanceCode ?? a.bracketCode ?? '',
          description: a.description ?? a.desc          ?? '',
        }));
        setAllowances(mapped);
      }
    } catch (error: any) {
      const errorMsg = error.response?.data?.message || error.message || 'Failed to load allowance codes.';
      setError(errorMsg);
  ...
  const endIndex         = Math.min(startIndex + PAGE_SIZE, filteredAllowances.length);
  const pagedAllowances  = filteredAllowances.slice(startIndex, endIndex);
      ...
      <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
          ...
          <div className="p-3 overflow-y-auto flex-1">
            <h3 className="text-blue-600 mb-2 text-sm">Allowance Code</h3>
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={2} className="px-3 py-4 text-center text-red-500 text-sm">
                        {error}
                      </td>
                    </tr>
                  ) : pagedAllowances.length === 0 ? (
                    <tr>
                      <td colSpan={2} className="px-3 py-4 text-center text-gray-500 text-sm">
                        No data available
                      </td>
                    ...
                    pagedAllowances.map((a, idx) => (
                      <tr
                        key={`${a.code}-${idx}`}
                        className="border-b border-gray-200 hover:bg-blue-50 cursor-pointer"
                        onClick={() => {
                          onSelect(a.code, a.description);
                          onClose();
                        }}
                      >
                        <td className="px-3 py-1.5">{a.code}</td>
          ...
          {!loading && !error && filteredAllowances.length > 0 && (
            <div className="flex-shrink-0 border-t border-gray-200">
              <Pagination
                currentPage={currentPage}
                totalCount={filteredAllowances.length}
                pageSize={PAGE_SIZE}
