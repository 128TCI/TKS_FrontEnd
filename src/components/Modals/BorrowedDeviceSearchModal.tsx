...
interface BorrowedDeviceSearchModalProps {
    show:          boolean;
    onClose:       () => void;
...
export function BorrowedDeviceSearchModal({
    show,
    onClose,
    onSelect,
    searchTerm,
    setSearchTerm,
}: BorrowedDeviceSearchModalProps) {
    const [devices,     setDevices]     = useState<{ code: string; description: string }[]>([]);
    ...
    const fetchDevices = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data) {
                const mapped = (response.data as any[]).map(d => ({
                    code:        d.code        ?? '',
            ...
            const msg = err.response?.data?.message || err.message || 'Failed to load devices.';
            setError(msg);
            console.error('Error fetching borrowed devices:', err);
        } finally {
            setLoading(false);
        }
    };
                        ...
                        <div className="border border-gray-200 rounded">
                            <table className="w-full border-collapse text-sm">
                                ...
                                <tbody>
                                    {loading ? (
                                        <tr>
                                            <td colSpan={2} className="px-3 py-4 text-center text-gray-500 text-sm">Loading...</td>
                                        </tr>
                                    ) : error ? (
                                        <tr>
                                            <td colSpan={2} className="px-3 py-4 text-center text-red-500 text-sm">{error}</td>
                                        </tr>
                                    ) : paged.length === 0 ? (
                                        <tr>
                                            <td colSpan={2} className="px-3 py-4 text-center text-gray-500 text-sm">No data available</td>
                                        </tr>
                                    ) : (
                                        paged.map((d, idx) => (
                                            <tr
                                                ...
                                                onClick={() => { onSelect(d.code, d.description); onClose(); }}
                                            >
