...
export function UpdateHoursPerWeekPage() {
  ...
  const [dateApplied, setDateApplied] = useState('5/5/2021');
  const [statusFilter, setStatusFilter] = useState<'active' | 'inactive' | 'all'>('active');
  const [showCalendar, setShowCalendar] = useState<'dateFrom1' | 'dateFrom2' | 'dateApplied' | null>(null);
  ...
  const handleCalendarClick = (field: 'dateFrom1' | 'dateFrom2' | 'dateApplied', event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleDateChange = (date: string) => {
    if (showCalendar === 'dateFrom1') {
      ...
      setDateFrom2(date);
    } else if (showCalendar === 'dateApplied') {
      setDateApplied(date);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {showCalendar && (
        <CalendarPopup
          value={showCalendar === 'dateFrom1' ? dateFrom1 : showCalendar === 'dateFrom2' ? dateFrom2 : dateApplied}
