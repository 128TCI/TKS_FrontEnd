...
export function ExportPayrollDataPage() {
  ...
  const handleToggle = (tab: string, value: string) => {
    const setterMap: Record<
      string,
      React.Dispatch<React.SetStateAction<string[]>>
    > = {
      "TK Group": setSelectedGroupCode,
      Branch: setSelectedBranches,
      Department: setSelectedDepartments,
      Division: setSelectedDivisions,
      "Group Schedule": setSelectedSchedules,
      "Pay House": setSelectedPayHouses,
      Section: setSelectedSections,
      Unit: setSelectedUnits,
    };
  ...
  const tabToSelectionMap: Record<string, string[]> = {
    "TK Group": selectedGroupCode,
    Branch: selectedBranches,
    Department: selectedDepartments,
    Division: selectedDivisions,
    "Group Schedule": selectedSchedules,
    "Pay House": selectedPayHouses,
    Section: selectedSections,
    Unit: selectedUnits,
  };

  const tabToSetterMap: Record<string, (codes: string[]) => void> = {
    "TK Group": setSelectedGroupCode,
    Branch: setSelectedBranches,
    Department: setSelectedDepartments,
    Division: setSelectedDivisions,
  ...
  const fetchEmployeeData = async (): Promise<EmployeeItem[]> => {
    const filters: GroupCodeFilters = {
      groupCode: selectedGroupCode.length ? selectedGroupCode : [""],
      braCode: selectedBranches.length ? selectedBranches : [""],
      depCode: selectedDepartments.length ? selectedDepartments : [""],
      divCode: selectedDivisions.length ? selectedDivisions : [""],
      grpCode: selectedSchedules.length ? selectedSchedules : [""],
      payCode: selectedPayHouses.length ? selectedPayHouses : [""],
      secCode: selectedSections.length ? selectedSections : [""],
      unitCode: selectedUnits.length ? selectedUnits : [""],
      active:
        statusFilter === "active"
          ? "1"
    ...
    const response = await apiClient.get(
      `Maintenance/EmployeeMasterFile/GetByGroupCodeAndStatus?${query.toString()}`,
    );

    if (!Array.isArray(response.data)) return [];

    // Normalize data for table
    const normalizedEmployees: EmployeeItem[] = response.data.map(
      (emp: any) => ({
        id: emp.empID,
        code: emp.empCode,
        name: `${emp.fName} ${emp.mName ? emp.mName + " " : ""}${emp.lName}${
  ...
  const filteredGroups = currentItems.filter(
    (item) =>
  ...
  const handleSelectAllGroups = () => {
    const selectedArray = tabToSelectionMap[activeTab] || [];
    const setter = tabToSetterMap[activeTab];
    if (!setter) return;

    const allCodes = filteredGroups.map((g) => g.code);

    const allSelected = allCodes.every((code) => selectedArray.includes(code));

    setter(allSelected ? [] : allCodes);
  };
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">
                    Export payroll and timekeeping data to external systems.
                    Select the date range and transaction date, then choose the
                    groups to export.
                  </p>
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                  ...
                  <table className="w-full">
                          ...
                          <input
                            ...
                            checked={
                              ...
                              filteredGroups.every((g) =>
                                (tabToSelectionMap[activeTab] || []).includes(
                                  g.code,
                                ),
                              )
                            }
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {paginatedGroups.map((item) => {
                        const selectedArray =
                          tabToSelectionMap[activeTab] || [];
                        const setter = tabToSetterMap[activeTab];

                        const isChecked = selectedArray.includes(item.code);

                        const toggleItem = () => {
                          if (!setter) return;
                          setter(
                            isChecked
                              ? selectedArray.filter((c) => c !== item.code)
                              : [...selectedArray, item.code],
                          );
                        };
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="text-gray-600 text-xs">
                    Showing{" "}
                    {filteredEmployees.length === 0 ? 0 : startGroupIndex + 1}{" "}
                    to {Math.min(endGroupIndex, filteredGroups.length)} of{" "}
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() =>
                        setCurrentEmpPage((p) => Math.max(1, p - 1))
                      }
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer whitespace-nowrap">
                      <input
                        ...
                        onChange={(e) =>
                          setApplyTransactionDate(e.target.checked)
                        }
                        className="w-4 h-4"
                      />
              ...
              <div className="space-y-6">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setProcessOptions({
                            ...processOptions,
                            leaveAbsences: e.target.checked,
                          })
                        }
                        className="w-4 h-4"
                      />
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setProcessOptions({
                            ...processOptions,
                            otherEarnings: e.target.checked,
                          })
                        }
                        className="w-4 h-4"
                      />
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setProcessOptions({
                            ...processOptions,
                            daysAndHours: e.target.checked,
                          })
                        }
                        className="w-4 h-4"
                      />
                      <span className="text-sm text-gray-700">
                        Days and No. of Hrs Worked
                      </span>
                    </label>
                  </div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) =>
                        handleSelectAllProcesses(e.target.checked)
                      }
                      className="w-4 h-4"
                    />
                    <span className="text-sm text-gray-700">
                ...
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                  <h3 className="text-gray-900 mb-4">
                    Compute As [Applies To Daily Paid Only]
                  </h3>
                  <div className="flex items-center gap-8">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
