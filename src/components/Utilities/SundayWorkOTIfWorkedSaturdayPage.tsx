...
export function SundayWorkOTIfWorkedSaturdayPage() {  const [activeTab,          setActiveTab]          = useState<TabName>('TK Group');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await apiClient.post('/Utilities/SundayPaidRegularHours', payload);
      if (ApiService.isApiSuccess(res)) {
        await showSuccessModal('Sunday Paid Hours settings successfully updated.');
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">Sunday Work is Considered OT if Worked on Saturday</h1>
          </div>
            ...
            <div className="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">Batch update Sunday work OT if worked on Saturday for selected employee groups. Configure adjustments across multiple employees efficiently.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {[
                      'Select groups and employees',
                      'Filter by employee status',
                      'Set date range for updates',
                      'Batch update Sunday OT records',
                    ].map(t => (
