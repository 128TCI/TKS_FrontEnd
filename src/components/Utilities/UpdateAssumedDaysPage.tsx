...
export function UpdateAssumedDaysPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const _ByUpdateResponse = await apiClient.post("/Utilities/UpdateAssumedDays", payload);
      ...
      if (isSuccessByUpdate) { 
        await showSuccessModal('Assumed Days successfully updated.', 2000);
        setSelectedGroups([]);
  ...
  const handleUnpost = async () => {
    if (!selectedEmployees.length) {
      await showErrorModal('Please select employee/s to unpost.');
      return;
    }
    if (!dateFrom || !dateTo) {
      await showErrorModal('Please select Date From and Date To.');
    ...
    try {
      setIsUpdating(true);
      await showSuccessModal('Assumed Days successfully unposted.');  
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">Update Assumed Days</h1>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">
                    Update assumed working days for selected employee groups within specified date ranges. Configure attendance assumptions for payroll processing and reporting.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="flex items-start gap-2">
                      <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                      <span className="text-gray-600">Bulk update processing</span>
                    </div>
                  </div>
                </div>
              </div>
