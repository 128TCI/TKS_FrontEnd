  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (ApiService.isApiSuccess(res)) {
        await showSuccessModal('Incomplete Logs 2 Shifts deleted successfully.');
        resetForm();
      }
    } catch {
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-indigo-400 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white font-semibold">Delete Incomplete Logs 2 Shifts In A Day</h1>
          </div>
                  ...
                  <div className="mt-4 flex items-center gap-6">
                    {(['active', 'inactive', 'all'] as const).map(s => (
                      <label key={s} className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="statusFilter2ShiftsLogs" value={s}
