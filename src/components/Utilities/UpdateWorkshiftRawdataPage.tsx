  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const [addTmpRes, updateRes] = await Promise.all([
        apiClient.post('/Utilities/UpdateWorkshiftRawData_AddtmpTable',  payload),
        apiClient.post('/Utilities/UpdateWorkshiftRawData_UpdateTKGroup', payload),
      ]);

      if (ApiService.isApiSuccess(addTmpRes) && ApiService.isApiSuccess(updateRes)) {
        await showSuccessModal('Successfully updated Workshift in Raw Data.');
        setSelectedItems([]);
        setDateFrom('');
        setDateTo('');
      } else {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {['Select groups and date range', 'Bulk update workshift data'].map(t => (
