...
import { Save, Users, Building2, Briefcase, Network, CalendarClock, Wallet, Clock, Check } from 'lucide-react';
...
export function NDBasicRoundDownPage() {  const [activeTab,          setActiveTab]          = useState<TabName>('TK Group');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await apiClient.post('/Utilities/NDBasicRoundDown', payload);
      if (ApiService.isApiSuccess(res)) {
        await showSuccessModal('ND Basic Round Down settings successfully updated.');
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">ND Basic Round Down</h1>
          </div>
          {/* Content Container */}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-2">
                    Apply ND (Night Differential) basic round down utility for selected employee groups. This tool helps standardize night differential calculations across employees.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    <div className="flex items-start gap-2">
                      <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                      <span className="text-gray-600">Select employees by group, branch, or department</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                      <span className="text-gray-600">Filter by active or inactive status</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                  ...
                  <div className="flex justify-end pt-4 border-t border-gray-200">
                    <button onClick={handleUpdate} disabled={isUpdating} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded transition-colors text-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
