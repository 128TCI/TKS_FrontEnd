...
export function Unpost2ShiftsTransactionPage() {  const [activeTab,          setActiveTab]          = useState<TabName>('TK Group');
  ...
  const uncheckAllOptions = () => setOptions({tardiness:false,otherEarnings:false,noOfDaysWork:false,selectAll:false,undertime:false,overtime:false,leaveAndAbsences:false,lateFiling:false});
  const handleOptionChange = (option: keyof typeof options) => {
    if (option==='selectAll') {
      const v=!options.selectAll;
      setOptions({tardiness:v,otherEarnings:v,noOfDaysWork:v,selectAll:v,undertime:v,overtime:v,leaveAndAbsences:v,lateFiling:v});
    } else { setOptions(prev=>({...prev,[option]:!prev[option]})); }
  };
  ...
  const resetForm = () => { setSelectedGroupsMap({...EMPTY_SELECTION}); setSelectedEmployees([]); setDateFrom(''); setDateTo(''); uncheckAllOptions(); };
  const handleAction = async () => {
    ...
    if (!Object.values(options).filter((_,i)=>i!==Object.keys(options).indexOf('selectAll')).some(Boolean) && !options.selectAll)
      { await showErrorModal('Please check at least 1 option to update.'); return; }
    try {
      setIsUpdating(true);
      const payload = {
      ...
      if (ApiService.isApiSuccess(res)) { await showSuccessModal('Successfully updated.'); resetForm(); }
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">Unpost 2 Shifts In A Day Transaction</h1>
          </div>
            ...
            <div className="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">Batch unpost 2-shifts-in-a-day transactions for selected employee groups. Select update options and configure the date range for processing.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {[
                      'Select groups and employees',
                      'Filter by employee status',
                      'Set date range for updates',
                      'Choose unpost options to apply',
                    ].map(t => (
                ...
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-3">
                      {(['tardiness','otherEarnings','noOfDaysWork','selectAll'] as const).map(key=>(
                        <label key={key} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={options[key]} onChange={()=>handleOptionChange(key)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"/>
                          <span className="text-sm text-gray-700">{key==='tardiness'?'Tardiness':key==='otherEarnings'?'Other Earnings':key==='noOfDaysWork'?'No. Of Days Work':'Select All'}</span>
                        </label>
                      ))}
                    </div>
                    <div className="space-y-3">
                      {(['undertime','overtime','leaveAndAbsences','lateFiling'] as const).map(key=>(
                        <label key={key} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={options[key]} onChange={()=>handleOptionChange(key)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"/>
                          <span className="text-sm text-gray-700">{key==='undertime'?'Undertime':key==='overtime'?'Overtime':key==='leaveAndAbsences'?'Leave and Absences':'Late Filing'}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  <div className="flex justify-end pt-4 border-t border-gray-200 mt-4">
                    <button onClick={handleAction} disabled={isUpdating} className="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors text-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
