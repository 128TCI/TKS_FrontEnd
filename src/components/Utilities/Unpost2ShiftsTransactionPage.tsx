...
import { Users, Building2, Briefcase, Network, CalendarClock, Wallet, RefreshCw } from 'lucide-react';
...
export function Unpost2ShiftsTransactionPage() {
  const formName = 'Unpost 2 Shifts In A Day Transaction';
  ...
  const handleUpdate = async () => {
    if (!selectedEmployees.length) {
      await Swal.fire({ icon: 'error', title: 'Error', text: 'Please select employee(s) to update.', timer: 2000, showConfirmButton: true });
      return;
    }
    if (!dateFrom || !dateTo) {
    ...
    if (noOptionSelected) {
      await Swal.fire({ icon: 'error', title: 'Error', text: 'Please check at least 1 option to update.', timer: 2000, showConfirmButton: true });
      return;
    }
    try {
      await Swal.fire({ icon: 'success', title: 'Success', text: 'Successfully updated.', timer: 2000, showConfirmButton: false });
      setSelectedGroups([]);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6 relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Right — Employee Table + Options + Date Range */}
