...
import { Trash2, Check, Users, Building2, Briefcase, CalendarClock, Wallet, Grid } from 'lucide-react';
...
export function DeleteOtRestdayAbsentPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await apiClient.post('/Utilities/DeleteOtRestdayAbsent', payload);
      if (ApiService.isApiSuccess(res)) { await showSuccessModal('Delete OT during restday successfully updated.'); resetForm(); }
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">Delete OT During Restday / Absent</h1>
          </div>
            ...
            <div className="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">Process Delete OT During Restday / Absent for selected employee groups within a specified date range.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {['Select groups and date range','Filter by employee status','Review records before processing','Update to apply changes'].map(t => (
                ...
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-5 space-y-4">
                  ...
                  <div className="flex justify-end pt-4 border-t border-gray-200">
                    <button onClick={handleUpdate} disabled={isUpdating}
                      ...
                      <Trash2 className="w-4 h-4" />{isUpdating ? 'Updating…' : 'Delete'}
                    </button>
                  </div>
                </div>
              </div>
