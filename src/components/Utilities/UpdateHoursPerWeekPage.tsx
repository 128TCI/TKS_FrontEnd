...
import { AlertCircle, Check, Save, RotateCcw, Users, Building2, Briefcase, Network, CalendarClock, Wallet, Grid, Box } from 'lucide-react';
...
export function UpdateHoursPerWeekPage() {
  ...
  const handleUpdate = async () => {
    ...
    if (!dateApplied)              { await showErrorModal('Please select Date Applied.'); return; }
    try {
      setIsUpdating(true);
      const payload = {
        ...
        DateTo:      toISO(dateTo),
        DateApplied: toISO(dateApplied),
      };
      const res = await apiClient.post('/Utilities/UpdateHoursPerWeek', payload);
  ...
  const handleUnpost = async () => {
    if (!selectedEmployees.length) { await showErrorModal('Please select employee/s to unpost.'); return; }
    if (!dateFrom || !dateTo)      { await showErrorModal('Please select Date From and Date To.'); return; }
    if (!dateApplied)              { await showErrorModal('Please select Date Applied.'); return; }
    try {
      setIsUpdating(true);
      ...
      const res = await apiClient.post('/Utilities/UpdateHoursPerWeek', payload);
      if (ApiService.isApiSuccess(res)) {
        await showSuccessModal('Successfully unposted No. of Hours Per Week.');
        resetForm();
      }
    } catch {
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg shadow-lg">
            <h1 className="text-white">Update No. of Hours Per Week</h1>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">
                    Update the number of hours per week for selected employee groups. Configure weekly hour requirements across multiple employees efficiently.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {['Select groups and employees', 'Filter by employee status',
                      'Specify effective dates', 'Update or unpost changes'].map(t => (
                ...
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <label className="text-sm text-gray-700 w-28">Date From:</label>
