...
export function DeleteRawdataPage() {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await Swal.fire({
      icon: 'warning', title: 'Confirm Delete', text: 'Are you sure you want to delete Employee Transaction?',
    ...
    try {
      ...
      const [res1, res2] = await Promise.all([
        apiClient.post('/Utilities/DeleteRawData_AddtmpDeleteRawData', payload),
        apiClient.post('/Utilities/DeleteRawData_DeleteLogs', payload),
      ]);
            ...
            <div className="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-3">Permanently delete raw attendance data for selected employee groups. Remove unprocessed time records from the system efficiently within a specified date range.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {['Select groups and date range to delete','Filter by employee status','Review selected records carefully','Deletion is permanent and cannot be undone'].map(t => (
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-700">This Process assumes that you really don't need the data in RAW DATA. You will not be able to UNDO the deletion after doing this process. You should could re-import again the DTR files.</p>
                    </div>
                  </div>
                </div>
              </div>
