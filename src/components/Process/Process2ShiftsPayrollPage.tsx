...
export function Process2ShiftsPayrollPage() {
  ...
  const [processOptions, setProcessOptions] = useState({
    tardiness: false, leaveAbsences: false, allowances: false,
    nightDifferentials: false, undertime: false, regularWorking: false,
    overtime: false, selectAll: false,
  });
  ...
  const handleOptionChange = (option: keyof typeof processOptions) => {
    if (option === 'selectAll') {
      ...
      setProcessOptions({
        tardiness: v, leaveAbsences: v, allowances: false,
        nightDifferentials: v, undertime: v, regularWorking: false,
        overtime: v, selectAll: v,
      });
    } else {
                  ...
                  <div className="flex">
                    {resultTabs.map(tab => (
                      <button key={tab.id} onClick={() => setActiveResultTab(tab.id)}
                        ...
                        {tab.label}
                        {activeResultTab === tab.id && activeResultData.length > 0 && (
                          <span className="ml-1.5 px-1.5 py-0.5 bg-blue-600 text-white rounded-full text-xs">{activeResultData.length}</span>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <div className="px-5 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between flex-wrap gap-2">
                  <span className="text-xs text-gray-500">
                    {filteredResultData.length === 0
                      ? 'No entries'
                      : `${(resultPage - 1) * resultPageSize + 1}–${Math.min(resultPage * resultPageSize, filteredResultData.length)} of ${filteredResultData.length} entries`}
