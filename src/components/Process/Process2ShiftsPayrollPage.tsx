...
const fetchFilteredEmployees = useCallback(async (
  ...
  try {
    const all = await fetchEmployeeData(status); // ← active/inactive/all filter applied here
...
const parseDateToISO = (dateStr: string): string => {
  ...
  // ── Result Tabs (7 only — no Holiday Pay) ────────────────────────────────
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                  <span>Showing {filteredEmployees.length === 0 ? 0 : startEmployeeIndex + 1} to {Math.min(endEmployeeIndex, filteredEmployees.length)} of {filteredEmployees.length} entries</span>
                ...
                <div className="mt-4 flex items-center gap-6">
                  {(['active', 'inactive', 'all'] as const).map(s => (
                    <label key={s} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="statusFilter2" value={s}
  ...
  <div className="flex items-end">
    <label className="flex items-center gap-2 cursor-pointer">
      ...
      <span className="text-sm text-gray-700">Include Late Filing</span>
              ...
              {/* Process Type Checkboxes — order per spec */}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3 pb-3 border-b border-blue-200">
                  {/* Row 1 */}
                  {([
                    ['tardiness',         'Tardiness'],
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={processOptions.selectAll} onChange={() => handleOptionChange('selectAll')}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span className="text-sm text-gray-900 font-medium">Select All</span>
                  </label>
                </div>
              </div>
