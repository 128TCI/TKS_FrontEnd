...
export function SystemConfigurationSetupPage() {
  ...
  const CustomCheckbox = ({
    ...
    <div
      ...
      className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all
        ...
        ${checked ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-400 hover:border-gray-500'}`}
    >
      {checked && <Check className="w-4 h-4 text-white stroke-[3]" />}
    </div>
  );

  const formatNumericInput = (value: string): string => value.replace(/[^\d]/g, '');
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        setFormData({
          ...
          noFlag_Break1: config.noFlag_Break1 != null ? String(config.noFlag_Break1) : '',
          noFlag_Break2: config.noFlag_Break2 != null ? String(config.noFlag_Break2) : '',
          noFlag_Break3: config.noFlag_Break3 != null ? String(config.noFlag_Break3) : '',
        });
      }
    } catch (error: any) {
      await Swal.fire({
  ...
  useEffect(() => {
    const handleEscKey = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && showEditMode) handleCancel();
    };
    document.addEventListener('keydown', handleEscKey);
    return () => document.removeEventListener('keydown', handleEscKey);
  }, [showEditMode, originalData]);

  const inputClass = "px-3 py-1.5 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100";
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-b-lg shadow-lg p-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-2">
                    Configure system-wide settings for time and attendance processing. Define device policies,
                    pairing rules, overtime calculations, and various system behaviors to match your organization's requirements.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    {[
                      'Device pairing policies and configurations',
                      'Overtime and tardiness process options',
                      'Break time and shift validation rules',
                      'System retry and interval settings',
                    ].map((t) => (
            ...
            <div className="mb-6">
              {!showEditMode && hasPermission('Edit') && (
                <button
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-green-600">During pairing of Breaks the system will ignore Multiple breaks when the difference of break is equal or less than the defined policy.</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className={`${inputClass} w-32`}
                      />
                      <p className="text-sm text-green-600 mt-1">This will be triggered when Midnight Shift is checked in workshift.</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className={`${inputClass} w-32`}
                      />
                      <p className="text-sm text-green-600 mt-1">This will be the count on retries when deadlock occurred.</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className={`${inputClass} w-32`}
                      />
                      <p className="text-sm text-green-600 mt-1">This will be the interval of system before retries when deadlock occurred.</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <input
                        ...
                        className={`${inputClass} w-32`}
                      />
                      <p className="text-sm text-green-600 mt-1">This is the number of minutes to consider as the pair of Break 2 Out, used in Device 5.</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <select
                        ...
                        className={`${inputClass} w-64`}
                      >
                        {['Device11','Device1','Device2','Device3','Device4','Device5','Device6',
                          'Device7','Device8','Device9','Device10','Device12','Device13','Device14',
                          'Device15','Device12_v810'].map((d) => (
                          <option key={d} value={d}>{d.replace('Device', 'Device ')}</option>
                        ))}
                      </select>
                ...
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <ul className="space-y-2 text-sm">
                    {[
                      ...
                      { dev: 'Device 4', desc: 'First Flag for BreakIn = Break1In, Second = Break2In, Third = Break3In, First Flag for BreakOut = Break1Out, Second = Break2Out, Third = Break3Out' },
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <h3 className="text-blue-700 mb-3 font-semibold">
                    Policy for Utility to Update the Time Flag Base on Set Policy of Breaks
                  </h3>
                  ...
                  <div className="bg-gray-100 p-3 rounded">
                    <div className="space-y-2">
                      {[
                        { label: 'Break1 (min)', field: 'noFlag_Break1' as const },
                        { label: 'Break2 (min)', field: 'noFlag_Break2' as const },
                        { label: 'Break3 (min)', field: 'noFlag_Break3' as const },
                      ].map(({ label, field }) => (
                        <div key={field} className="flex items-center gap-3">
                          ...
                          <input
                            ...
                            onChange={(e) => setFormData({ ...formData, [field]: formatNumericInput(e.target.value) })}
                            disabled={!showEditMode}
                            placeholder="minutes"
                            className={`${inputClass} w-32`}
                          />
