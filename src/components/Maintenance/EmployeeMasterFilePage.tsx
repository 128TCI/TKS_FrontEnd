...
export function EmployeeMasterFilePage() {
  ...
  const validateGovIdUniqueness = (
    ...
    const otherEmployees = currentEmpCode
...
const fetchEmployees = async () => {
  ...
  const convertAPIToFormData = (employee: Employee) => {
    setFormData({
      ...
      dateHired: toDateOnly(employee.dateHired),
      regularized: toDateOnly(employee.dateRegularized),
      probeStart: toDateOnly(employee.probeStart),
      probeEnd: toDateOnly(employee.probeEnd),
      dateResigned: toDateOnly(employee.dateResigned),
      suspended: toDateOnly(employee.dateSuspended),
  ...
  const handlePhotoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!file.type.startsWith('image/')) {
        Swal.fire({ icon: 'error', title: 'Invalid File', text: 'Please select an image file (JPG, PNG, etc.)' });
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (!formData.firstName.trim()) {
      await Swal.fire({ icon: 'warning', title: 'Validation Error', text: 'First Name is required.' });
      return;
    }
    if (!formData.lastName.trim()) {
      await Swal.fire({ icon: 'warning', title: 'Validation Error', text: 'Last Name is required.' });
      return;
    }
  ...
  const handleEmployeeSearchSelect = async (empCode: string, name: string) => {
    try {
      ...
      if (!employee) {
        await Swal.fire({ icon: 'error', title: 'Not Found', text: 'Employee not found.' });
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-700 mb-2">
                    Manage employee records including personal information, employment details, organizational assignments, and government ID numbers.
                  </p>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div>
                        ...
                        <InlineDatePicker date={formData.dateHired} onChange={(date) => setFormData({...formData, dateHired: date})} disabled={!isEditing} />
                      </div>
                      <div>
                        <label className="block text-gray-700 font-bold text-sm mb-2">Regularized</label>
                        <InlineDatePicker date={formData.regularized} onChange={(date) => setFormData({...formData, regularized: date})} disabled={!isEditing} />
                      </div>
                      <div className="flex items-center h-[42px]">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={formData.contractual} onChange={(e) => setFormData({...formData, contractual: e.target.checked})} disabled={!isEditing} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div>
                        ...
                        <InlineDatePicker date={formData.probeStart} onChange={(date) => setFormData({...formData, probeStart: date})} disabled={!isEditing} />
                      </div>
                      <div>
                        <label className="block text-gray-700 font-bold text-sm mb-2">Probe End</label>
                        <InlineDatePicker date={formData.probeEnd} onChange={(date) => setFormData({...formData, probeEnd: date})} disabled={!isEditing} />
                      </div>
                      <div className="flex items-center h-[42px]">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={formData.separated} onChange={(e) => setFormData({...formData, separated: e.target.checked})} disabled={!isEditing} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div>
                        ...
                        <InlineDatePicker date={formData.dateResigned} onChange={(date) => setFormData({...formData, dateResigned: date})} disabled={!isEditing} />
                      </div>
                      <div>
                        <label className="block text-gray-700 font-bold text-sm mb-2">Suspended</label>
                        <InlineDatePicker date={formData.suspended} onChange={(date) => setFormData({...formData, suspended: date})} disabled={!isEditing} />
                      </div>
                      <div className="flex items-center h-[42px]">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={formData.suspend} onChange={(e) => setFormData({...formData, suspend: e.target.checked})} disabled={!isEditing} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500" />
                ...
                <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.branch} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowBranchModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, branch: '', branchCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.division} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowDivisionModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, division: '', divisionCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.department} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowDepartmentModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, department: '', departmentCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.section} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowSectionModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, section: '', sectionCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.unit} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowUnitModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, unit: '', unitCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {isEditing && (
                          <>
                            <button onClick={() => setShowPayHouseModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, payHouse: '', payHouseCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.area} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowAreaModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, area: '', areaCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.location} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowLocationModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, location: '', locationCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input type="text" value={formData.onlineApplication} disabled className="flex-1 px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        {isEditing && (
                          <>
                            <button onClick={() => setShowOnlineApprovalModal(true)} className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"><Search className="w-4 h-4" /></button>
                            <button onClick={() => setFormData({...formData, onlineApplication: '', onlineAppCode: ''})} className="px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"><X className="w-4 h-4" /></button>
                          </>
                        )}
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <textarea value={formData.homeAddress} onChange={(e) => setFormData({...formData, homeAddress: e.target.value})} disabled={!isEditing} rows={3} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 resize-none" />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Present Address</label>
                          <textarea value={formData.presentAddress} onChange={(e) => setFormData({...formData, presentAddress: e.target.value})} disabled={!isEditing} rows={3} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 resize-none" />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <label className="block text-gray-700 font-bold text-sm mb-2">City</label>
                            <input type="text" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                          </div>
                          <div>
                            <label className="block text-gray-700 font-bold text-sm mb-2">Province</label>
                            <input type="text" value={formData.province} onChange={(e) => setFormData({...formData, province: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                          </div>
                          <div>
                            <label className="block text-gray-700 font-bold text-sm mb-2">Postal Code</label>
                            <input type="text" value={formData.postalCode} onChange={(e) => setFormData({...formData, postalCode: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" value={formData.civilStatus} onChange={(e) => setFormData({...formData, civilStatus: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Citizenship</label>
                          <input type="text" value={formData.citizenship} onChange={(e) => setFormData({...formData, citizenship: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Religion</label>
                          <input type="text" value={formData.religion} onChange={(e) => setFormData({...formData, religion: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-gray-700 font-bold text-sm mb-2">Weight</label>
                            <input type="text" value={formData.weight} onChange={(e) => setFormData({...formData, weight: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                          </div>
                          <div>
                            <label className="block text-gray-700 font-bold text-sm mb-2">Height</label>
                            <input type="text" value={formData.height} onChange={(e) => setFormData({...formData, height: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                          </div>
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Gender</label>
                          <select value={formData.gender} onChange={(e) => setFormData({...formData, gender: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60">
                        ...
                        <div>
                          ...
                          <input type="text" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Birth Date</label>
                          <InlineDatePicker date={formData.birthDate} onChange={(date) => setFormData({...formData, birthDate: date})} disabled={!isEditing} />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Age</label>
                          <input type="text" value={formData.age} onChange={(e) => setFormData({...formData, age: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                        <div>
                          <label className="block text-gray-700 font-bold text-sm mb-2">Birth Place</label>
                          <input type="text" value={formData.birthPlace} onChange={(e) => setFormData({...formData, birthPlace: e.target.value})} disabled={!isEditing} className="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60" />
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      ...
                      <input ref={fileInputRef} type="file" accept="image/*" onChange={handlePhotoSelect} disabled={!isEditing} className="hidden" />
                      <div
                        className={`relative flex items-center justify-center w-full h-64 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg border-2 border-dashed border-blue-300 overflow-hidden ${isEditing ? 'cursor-pointer hover:border-blue-500 transition-colors' : ''}`}
                      ...
                      >
                        {photoPreview ? (
                          ...
                          <div className="flex flex-col items-center justify-center text-blue-400">
                            ...
                            {isEditing && <p className="text-sm text-blue-600">Click to upload photo</p>}
                          </div>
                        )}
                      </div>
                      {isEditing && (
                        <div className="mt-4 space-y-2">
                          <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2">
                            <Upload className="w-4 h-4" />
                            {photoPreview ? 'Change Photo' : 'Upload Photo'}
                          </button>
                          {(photoPreview || photoFile) && (
                            <button type="button" onClick={handleRemovePhoto} className="w-full px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors flex items-center justify-center gap-2">
                              <X className="w-4 h-4" />
                              Remove Photo
                            </button>
                          )}
                          <p className="text-xs text-gray-500 text-center mt-2">Max size: 5MB • Formats: JPG, PNG, GIF</p>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden pointer-events-auto">
            <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
              ...
              <button onClick={() => setShowEmpStatModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
              <h3 className="text-blue-600 mb-4">Emp Stat Code</h3>
              <div className="flex items-center gap-2 mb-4">
                <label className="text-gray-700">Search:</label>
                <input type="text" value={empStatSearchTerm} onChange={(e) => setEmpStatSearchTerm(e.target.value)} placeholder="Type status code or description..." className="flex-1 px-3 py-1.5 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              {loadingEmpStatuses ? (
                      ...
                      <tbody>
                        {filteredEmpStatuses.length > 0 ? (
                          filteredEmpStatuses.map((status, index) => (
                            <tr key={status.code} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                              onClick={() => { setFormData({...formData, empStatCode: status.code}); setShowEmpStatModal(false); setEmpStatSearchTerm(''); }}>
                              <td className="px-4 py-2 text-gray-800">{status.code}</td>
                              <td className="px-4 py-2 text-gray-800">{status.description}</td>
                            </tr>
                          ))
                        ) : (
                          <tr><td colSpan={2} className="px-4 py-8 text-center text-gray-500">No employee statuses found</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
      ...
      {showBranchModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowBranchModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                ...
                <button onClick={() => setShowBranchModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Branch</h3>
                {loadingBranches ? <div className="text-center py-8">Loading branches...</div> : branchError ? <div className="text-center py-8 text-red-600">{branchError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Manager</th></tr></thead>
                      <tbody>
                        {branchList.length > 0 ? branchList.map((branch, index) => (
                          <tr key={branch.branchId} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, branch: branch.description, branchCode: branch.code}); setShowBranchModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No branches found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showDivisionModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowDivisionModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Division</h2>
                <button onClick={() => setShowDivisionModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Division</h3>
                {loadingDivisions ? <div className="text-center py-8">Loading divisions...</div> : divisionError ? <div className="text-center py-8 text-red-600">{divisionError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {divisionList.length > 0 ? divisionList.map((division, index) => (
                          <tr key={division.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, division: division.description, divisionCode: division.code}); setShowDivisionModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No divisions found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showDepartmentModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowDepartmentModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                ...
                <button onClick={() => setShowDepartmentModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Department</h3>
                {loadingDepartments ? <div className="text-center py-8">Loading departments...</div> : departmentError ? <div className="text-center py-8 text-red-600">{departmentError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th><th className="px-4 py-2 text-left text-gray-700">Division</th></tr></thead>
                      <tbody>
                        {departmentList.length > 0 ? departmentList.map((dept, index) => (
                          <tr key={dept.departmentId} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, department: dept.description, departmentCode: dept.code}); setShowDepartmentModal(false); }}>
      ...
      {showSectionModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowSectionModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Section</h2>
                <button onClick={() => setShowSectionModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Section</h3>
                {loadingSections ? <div className="text-center py-8">Loading sections...</div> : sectionError ? <div className="text-center py-8 text-red-600">{sectionError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {sectionList.length > 0 ? sectionList.map((section, index) => (
                          <tr key={section.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, section: section.description, sectionCode: section.code}); setShowSectionModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No sections found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showUnitModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowUnitModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Unit</h2>
                <button onClick={() => setShowUnitModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Unit</h3>
                {loadingUnits ? <div className="text-center py-8">Loading units...</div> : unitError ? <div className="text-center py-8 text-red-600">{unitError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {unitList.length > 0 ? unitList.map((unit, index) => (
                          <tr key={unit.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, unit: unit.description, unitCode: unit.code}); setShowUnitModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No units found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showPayHouseModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowPayHouseModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Pay House</h2>
                <button onClick={() => setShowPayHouseModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Pay House</h3>
                {loadingPayHouses ? <div className="text-center py-8">Loading pay houses...</div> : payHouseError ? <div className="text-center py-8 text-red-600">{payHouseError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {payHouseList.length > 0 ? payHouseList.map((payHouse, index) => (
                          <tr key={payHouse.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, payHouse: payHouse.description, payHouseCode: payHouse.code}); setShowPayHouseModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No pay houses found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showAreaModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowAreaModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Area</h2>
                <button onClick={() => setShowAreaModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Area</h3>
                {loadingAreas ? <div className="text-center py-8">Loading areas...</div> : areaError ? <div className="text-center py-8 text-red-600">{areaError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {areaList.length > 0 ? areaList.map((area, index) => (
                          <tr key={area.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, area: area.description, areaCode: area.code}); setShowAreaModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No areas found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showLocationModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowLocationModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Location</h2>
                <button onClick={() => setShowLocationModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Location</h3>
                {loadingLocations ? <div className="text-center py-8">Loading locations...</div> : locationError ? <div className="text-center py-8 text-red-600">{locationError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Head</th></tr></thead>
                      <tbody>
                        {locationList.length > 0 ? locationList.map((location, index) => (
                          <tr key={location.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, location: location.description, locationCode: location.code}); setShowLocationModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No locations found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showOnlineApprovalModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowOnlineApprovalModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Online Approval</h2>
                <button onClick={() => setShowOnlineApprovalModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Online Approval</h3>
                {loadingOnlineApprovals ? <div className="text-center py-8">Loading online approvals...</div> : onlineApprovalError ? <div className="text-center py-8 text-red-600">{onlineApprovalError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th><th className="px-4 py-2 text-left text-gray-700">Manager</th></tr></thead>
                      <tbody>
                        {onlineApprovalList.length > 0 ? onlineApprovalList.map((approval, index) => (
                          <tr key={approval.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, onlineApplication: approval.description, onlineAppCode: approval.code}); setShowOnlineApprovalModal(false); }}>
                        ...
                        )) : <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-500">No online approvals found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showJobLevelModal && (
        <>
          <div className="fixed inset-0 z-[99] bg-black bg-opacity-50" onClick={() => setShowJobLevelModal(false)} />
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 pointer-events-none">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden pointer-events-auto">
              <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b border-gray-300">
                <h2 className="text-gray-800">Search Job Level</h2>
                <button onClick={() => setShowJobLevelModal(false)} className="text-gray-600 hover:text-gray-900 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-6 max-h-[calc(85vh-60px)] overflow-y-auto">
                <h3 className="text-blue-600 mb-4">Job Level</h3>
                {loadingJobLevels ? <div className="text-center py-8">Loading job levels...</div> : jobLevelError ? <div className="text-center py-8 text-red-600">{jobLevelError}</div> : (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <table className="w-full">
                      <thead><tr className="bg-gray-100 border-b border-gray-300"><th className="px-4 py-2 text-left text-gray-700">Code</th><th className="px-4 py-2 text-left text-gray-700">Description</th></tr></thead>
                      <tbody>
                        {jobLevelList.length > 0 ? jobLevelList.map((jobLevel, index) => (
                          <tr key={jobLevel.id} className={`border-b border-gray-200 hover:bg-blue-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                            onClick={() => { setFormData({...formData, jobLevel: jobLevel.description, jobLevelCode: jobLevel.code}); setShowJobLevelModal(false); }}>
                            <td className="px-4 py-2 text-gray-800">{jobLevel.code}</td>
                            <td className="px-4 py-2 text-gray-800">{jobLevel.description}</td>
                          </tr>
                        )) : <tr><td colSpan={2} className="px-4 py-8 text-center text-gray-500">No job levels found</td></tr>}
                      </tbody>
                    </table>
                  </div>
                )}
      ...
      {showDeleteConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md">
            ...
            <div className="p-6">
              <p className="text-gray-800 mb-2">Are you sure you want to delete this employee?</p>
              <p className="text-sm text-gray-500 mb-6">
                This action will be blocked if the employee is referenced in processed records, organizational setups, or raw data.
              </p>
              <div className="flex items-center justify-end gap-3">
                <button onClick={handleConfirmDelete} className="px-8 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors shadow-sm">OK</button>
                <button onClick={handleCancelDelete} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-full hover:bg-gray-300 transition-colors shadow-sm">Cancel</button>
              </div>
            </div>
          </div>
        </div>
